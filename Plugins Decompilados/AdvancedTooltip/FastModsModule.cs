...
// Type: AdvancedTooltip.FastModsModule
    ...
    public void DrawUiHoverFastMods(Element tooltip)
    {
      try
      {
        ...
        for (int index = 0; index < this._mods.Count; index = index + (mod.ModLines - 1) + 1)
        {
          ...
          if (ToggleNode.op_Implicit(this._modsSettings.EnableFastModsTags))
          {
      ...
      foreach (string input in strArray1)
      {
        if (!input.StartsWith("<italic>", StringComparison.Ordinal))
        {
          if (input.StartsWith("<smaller>", StringComparison.Ordinal) || input.StartsWith("<crafted>", StringComparison.Ordinal))
          {
            ...
            Color color1 = ColorNode.op_Implicit(flag1 ? this._modsSettings.PrefixColor : this._modsSettings.SuffixColor);
            bool flag3 = false;
            ...
            if (startIndex != -1 && (int.TryParse(input.Substring(startIndex, 2), out result) || int.TryParse(input.Substring(startIndex, 1), out result)))
            {
              ...
              {
                case 1:
                  color2 = ColorNode.op_Implicit(this._modsSettings.T1Color);
                  break;
                case 2:
                  color2 = ColorNode.op_Implicit(this._modsSettings.T2Color);
                  break;
                case 3:
                  color2 = ColorNode.op_Implicit(this._modsSettings.T3Color);
                  break;
                default:
                  color2 = color1;
                  break;
...
label_55:
      ...
      foreach (string input in strArray2)
      {
        ...
        if (!flag)
        {
          DebugWindow.LogMsg("Cannot extract mod from parsed mods: " + str, 4f);
